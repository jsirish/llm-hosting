#!/bin/bash
# Qwen2.5-Coder-1.5B Fast Compecho "⏳ Loading model (this takes ~1 minute)..."
echo "   You can monitor progress with: tail -f $LOG_FILE"
echo ""

# Start vLLM server (use absolute path on RunPod)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
"${SCRIPT_DIR}/../scripts/start-vllm-server.sh" 2>&1 | tee "$LOG_FILE" Model - vLLM Server Launcher
...
echo "⚡ Starting Qwen Fast Completion Model (1.5B)..."
echo "==============================================="
...
echo "   Log: $LOG_FILE"
echo ""
echo "⏳ Loading model (this takes 1-2 minutes)..."
echo "   You can monitor progress with: tail -f $LOG_FILE"
echo ""
